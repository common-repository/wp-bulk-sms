...
use WBS_WP_SMS_TO_BULK\Admin\WBS_SMS_TO_Helper;
...
class WBS_SMS_TO_Privacy_Actions {
	...
	function on_screen_layout_columns( $columns, $screen ) {
		if ( strpos( $screen, 'wp-sms-to-bulk-subscribers-privacy' ) !== false ) {
			$columns[ $screen ] = 2;
		}
	...
	public function admin_notification() {
		 ...
		 * privacy Page
		 */
		if ( $pagenow == "admin.php" and isset( $_GET['page'] ) AND sanitize_text_field($_GET['page']) == "wp-sms-to-bulk-subscribers-privacy" ) {
			...
			if ( isset( $_GET['error'] ) ) {
				...
				if ( sanitize_text_field($_GET['error']) == "empty_number" ) {
					WBS_SMS_TO_Helper::notice( __( 'Please enter the mobile phone number', 'wp-sms-to-bulk' ), "error" );
				}
				...
				*  Not found User
				...
				if ( sanitize_text_field($_GET['error']) == "not_found" ) {
					WBS_SMS_TO_Helper::notice( __( 'User with this mobile phone number was not found', 'wp-sms-to-bulk' ), "error" );
				}
			}
			...
			if ( isset( $_GET['delete_mobile_phone'] ) ) {
				WBS_SMS_TO_Helper::notice( sprintf( __( 'Mobile phone number %s is removed completely', 'wp-sms-to-bulk' ), trim( sanitize_text_field($_GET['delete_mobile_phone'] )) ), "success" );
			}
	...
	/*
	 * Process WBS_SMS_TO_Privacy Form
	 *
	 */
	public function process_form() {
		if ( isset( $_POST['sms_to_bulk_nonce_privacy'] ) and isset( $_POST['submit'] ) and ( isset( $_POST['mobile_phone-number-delete'] ) || isset( $_POST['mobile_phone-number-export'] ) ) ) {
			if ( wp_verify_nonce( sanitize_text_field($_POST['sms_to_bulk_nonce_privacy']), 'sms_to_bulk_nonce_privacy' ) ) {
				...
				if ( sanitize_text_field($_POST['submit']) == __( 'Export' ) ) {
					$this->create_csv( $user_data, "wp-sms-to-bulk-report-" . $mobile_phone );
				}
				...
				if ( sanitize_text_field($_POST['submit']) == __( 'Delete' ) ) {
					wp_redirect( admin_url( add_query_arg( array( 'page' => 'wp-sms-to-bulk-subscribers-privacy', 'delete_mobile_phone' => $mobile_phone ), 'admin.php' ) ) );
					exit;
				}
			}
		}
	...
	public function check_empty_mobile_phone( $mobile_phone ) {
		if ( empty( $mobile_phone ) ) {
			wp_redirect( admin_url( add_query_arg( array( 'page' => 'wp-sms-to-bulk-subscribers-privacy', 'error' => 'empty_number' ), 'admin.php' ) ) );
			exit;
		}
	}
    ...
    function wbs_get_users_with_phone($key_item) {
        ...
        if (($mobile_phone)) {
            $user = wbs_get_user_by_mobile_phone_bulk('mobile_phone', $mobile_phone);
            $users[0] = $user;
        }
        return $users;
    }
	...
	public function check_user_exist_mobile_phone( $mobile_phone ) {
		 ...
		 */
                $get_user = $this->wbs_get_users_with_phone($mobile_phone);                
		...
		if ( empty( $result ) ) {
			wp_redirect( admin_url( add_query_arg( array( 'page' => 'wp-sms-to-bulk-subscribers-privacy', 'error' => 'not_found' ), 'admin.php' ) ) );
			exit;
		}
...
new WBS_SMS_TO_Privacy_Actions();